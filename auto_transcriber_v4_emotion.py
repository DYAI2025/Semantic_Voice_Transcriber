...
from services.transcription_service.transcription_service import (
    mark_low_confidence_segments as service_mark_low_confidence_segments,
    transcribe_with_whisper as service_transcribe_with_whisper,
)
...
def transcribe_with_whisper(
    ...
    use_audio_chunking: bool = True,
    chunk_duration: float = 300.0,
    overlap_duration: float = 5.0,
    initial_prompt: Optional[str] = None
) -> Dict[str, Any]:
    """
    Delegate Whisper transcription to the isolated transcription service.

    Legacy parameters are accepted for compatibility but handled by optional
    adapters rather than direct imports to keep the core transcription isolated.
    """
    logger.info("Routing transcription through services.transcription_service")
    return service_transcribe_with_whisper(
        audio_path,
...
mark_low_confidence_segments = service_mark_low_confidence_segments
